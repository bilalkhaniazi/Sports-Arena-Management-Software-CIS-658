...
const formatRoleLabel = (role?: string, title?: string) => {
  if (role) {
...
const DropdownUser = () => {
  ...
  const [user, setUser] = useState<{ name: string; title?: string; role?: string } | null>(null);
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        ...
        <span className="block text-sm font-medium text-black dark:text-white">
          ...
          <span className="block text-xs text-body-color dark:text-slate-300">
            {formatRoleLabel(user?.role, user?.title)}
          </span>
        </span>
        ...
        >
          <ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
            <li>
              <Link
                to="/profile"
                className="flex items-center gap-3.5 text-sm font-medium text-slate-900 duration-300 ease-in-out hover:text-primary dark:text-slate-100 lg:text-base"
              >
                <svg
          ...
          <button
            type="button"
            className="flex w-full items-center gap-3.5 px-6 py-4 text-left text-sm font-medium text-slate-900 duration-300 ease-in-out hover:text-primary dark:text-slate-100 lg:text-base"
